...
bool ParticleSystem::InitParticleSystem(const std::string& updateVertShader, const std::string& updateGeoShader, const std::string& renderVertShader, const std::string& renderGeoShader, const std::string& renderFragShader)
{
	...
	mUpdateShader.load(updateVertShader.c_str(), nullptr, updateGeoShader.c_str());
	...
	initParticle.Type = Particle::Type::GENERATOR;
	...
	for (int i = 0; i < sBufferSize; i++)
	{
		...
		glVertexAttribPointer(5, 1, GL_INT, GL_FALSE, sizeof(Particle), (void*)44);
	}
	mCurrentReadBuffer = 0;
	mNumParticles = 1;
...
void ParticleSystem::UpdateParticles(float timeStep)
{
	...
	glBeginTransformFeedback(GL_POINTS);	// Throws invalid operation error
	Util::PrintErrors();
...
void ParticleSystem::SetMatrices(const glm::mat4& projection, const glm::vec3& eye, const glm::vec3& view, const glm::vec3& upVector)
{
	mProjection = projection;
	mView = glm::lookAt(eye, view, upVector);
